...
disp("cov_LOST_init = ")
disp(cov_LOST_init)
disp("cov_LOST_end (h, lon, lat) = ") % from c++
cov_LOST_end_lonlat = [ 0.000690216  6.48873e-07   -2.406e-07
                        6.48873e-07  8.44696e-10 -2.68466e-10
                        -2.406e-07 -2.68466e-10  1.34741e-10];
...
disp("cov_DLT_end (h, lon, lat) = ") % from c++
cov_DLT_end_lonlat = [  0.00099847  9.11541e-07  -1.3444e-07
                        9.11541e-07  1.25245e-09 -1.80429e-10
                        -1.3444e-07 -1.80429e-10  2.01104e-10];
...
plot(lon_to_loc(xlost), lat_to_loc(ylost), '-', 'Color','#ffa500', 'LineWidth',3)
plot(lon_to_loc(xdlt), lat_to_loc(ydlt), '--', 'Color','#cc7000', 'LineWidth',3)

mc_lost = table2array(readtable("../output/MC_LOST_entry_canted.csv"));
x = lon_to_loc(mc_lost(1:300,2)*180/pi);
y = lat_to_loc(mc_lost(1:300,3)*180/pi);

scatter(x, y, '+', MarkerEdgeColor='#ffa500')

mc_dlt = table2array(readtable("../output/MC_DLT_entry_canted.csv"));
x = lon_to_loc(mc_dlt(1:300,2)*180/pi);
y = lat_to_loc(mc_dlt(1:300,3)*180/pi);

scatter(x, y, '*', MarkerEdgeColor='#cc7000')
% landing site
...
% uncomment to have the cov of the monte carlo
% C = cov(mc_dlt);
% disp('C')
% getErrorEllipse([lonf, latf], C(2:3,2:3)*(180/pi)^2, 0.99);
